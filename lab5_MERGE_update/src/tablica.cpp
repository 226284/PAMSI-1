...
tablica::tablica(int podany_rozmiar) {
	...
	this->pom = new int[podany_rozmiar];
}
...
int tablica::odczytanie(int pozycja, string* blad) {
	...
	if (pozycja <= rozm) {
		...
		cout
				<< "probujesz odczytac pozycje znajdujaca sie poza zakresem tablicy"
				<< endl;
	}
...
void tablica::licze_srednia(int ilosc_powtorzen, int ilosc_elementow,
		string* blad) {
	chrono_typ srednia;
...
void tablica::wypelniam_tablice_malejaco(int ilosc_elementow, string* blad) {

	for (int i = 0; i < ilosc_elementow; i++) {
		this->przypisanie(i, ilosc_elementow - 1 - i, blad);
	}
...
int tablica::piwot_wartosc_mediana(int lewo, int prawo) {
	...
	if (wsk[lewo] >= wsk[(lewo + prawo) / 2] && wsk[lewo] <= wsk[prawo]
			|| wsk[lewo] <= wsk[(lewo + prawo) / 2]
					&& wsk[lewo] >= wsk[prawo]) {
		return wsk[lewo];
	}

	if (wsk[prawo] >= wsk[(lewo + prawo) / 2] && wsk[prawo] <= wsk[lewo]
			|| wsk[prawo] <= wsk[(lewo + prawo) / 2]
					&& wsk[prawo] >= wsk[lewo]) {
		return wsk[prawo];
	}

	if (wsk[(lewo + prawo) / 2] >= wsk[lewo]
			&& wsk[(lewo + prawo) / 2] <= wsk[prawo]
			|| wsk[(lewo + prawo) / 2] <= wsk[lewo]
					&& wsk[(lewo + prawo) / 2] >= wsk[prawo]) {
...
void tablica::scal(int lewo, int srodek, int prawo) {
	int i;
	int j;
	int k;
...
void tablica::mergesort(int lewo, int prawo) {
	int srodek;

	if (lewo < prawo) {

		srodek = (prawo + lewo) / 2; //środek tablicy
...
void tablica::czas_merge(int ilosc_powtorzen, string* blad) {
	chrono_typ srednia;
	int lewo = 0;
	int prawo = rozm - 1;

	for (int i = 0; i < ilosc_powtorzen; i++) {
		this->wypelniam_tablice_malejaco(rozm, blad);
		auto poczatek = high_resolution_clock::now();
		this->mergesort(lewo, prawo);
		auto koniec = high_resolution_clock::now();
		auto roznica = duration_cast < microseconds > (koniec - poczatek);
